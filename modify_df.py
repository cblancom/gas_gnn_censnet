...
def modify_values(df, column_list, pct_modification = args.argument_random):
  ...
  noise = np.random.uniform(low=-pct_modification, high=pct_modification, size=len(df))
...
def randomize_values(file_path, savepath):
    node_info = modify_values(pd.read_excel(file_path, sheet_name='node.info'),
    ['Pmax', 'Pmin'])

    node_dem = modify_values(pd.read_excel(file_path, sheet_name='node.dem'),
    ['Res', 'Ind'])
    ...
    well_df = modify_values(pd.read_excel(file_path, sheet_name='well'),
    ['Imax', 'Imin'])

    pipe_df = modify_values(pd.read_excel(file_path, sheet_name='pipe'),
    ['Kij', 'Fg_max', 'Fg_min'])

    comp_df = modify_values(pd.read_excel(file_path, sheet_name='comp'),
    ['ratio', 'fmaxc'])
...
for i in range(1, trials):
    try:
        savepath = './'+savepath_folder+'/results_random_df/ng_case_col_random_'+str(i)+'.xlsx'
        randomize_values(gas_file, savepath)
        path_power = './db/SIN_col_63.xlsx'
        name = 'gnn_samples'
        ...
        if p.m.options.objfcnval > 1: 
            file_ = open('./'+savepath_folder+'/results_pkl/'+'gnn_sample_'+str(i)+'.pkl', 'wb')
            dill.dump(p, file_)
            file_.close()
